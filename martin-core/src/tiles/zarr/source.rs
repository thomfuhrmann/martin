...
use tracing::warn;
use zarrs::hierarchy::Hierarchy;

use crate::tiles::zarr::error::ZarrError;
use crate::tiles::zarr::model::ModelInfo;
...
impl ZarrSource {
    ...
    pub fn new(id: String, path: PathBuf) -> Result<Self, ZarrError> {
        ...
        let tif_file =
            File::open(&path).map_err(|e: std::io::Error| ZarrError::IoError(e, path.clone()))?;
        
        let zarr_store = FilesystemStore::new(&path).unwrap();
        zarr_store.list();
        // let temp_array = Array::new(store.clone(), "/temp")?;

        // // 1. Get the CF attributes manually
        // let attributes = temp_array.attributes();
        // let coords = attributes.get("coordinates"); // "y x"

        // // 2. Load the coordinate arrays
        // let x_array = Array::new(store.clone(), "/x")?;
        // let y_array = Array::new(store.clone(), "/y")?;
